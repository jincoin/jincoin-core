...
#if defined(NDEBUG)
# error "Jincoin cannot be compiled without assertions."
#endif
                              ...
                              std::vector<uint256>& vHashTxnToUncache)
{
...

    {
        ...
        {
            LogPrint("mempool", "replacing tx %s with %s for %s JIN additional fees, %d delta bytes\n",
...
//
// JIN check header auxpow proof of work
...
// JIN: block header template's for auxpow integration
template<typename T>
...
CAmount GetBlockSubsidy(int nHeight, const Consensus::Params& consensusParams)
{
    ...
    CAmount nSubsidy = 192.96 * COIN;

    if (nHeight < consensusParams.nSubsidyIncreaseHeight) {
        // Force block reward to zero when right shift is undefined.
        if (halvings >= 64)
            return 0;

        // Subsidy is cut in half every 25810 blocks which will occur approximately every 24 days.
        nSubsidy >>= halvings;
        return nSubsidy;
    }
    else {
        // Subsidy is fixed at 1 COIN and halvings no longer occur.
        return 1 * COIN;
    }
}
...
{
    // JIN check auxpow first then fallback to normal check
